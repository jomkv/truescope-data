...
from data_cleaning.verdict_normalizer import normalize_verdict
...
class PolitifactFactcheckEmbedding(BaseEmbedding):
    ...
    def process(self):
        ...
        for article in article_dicts:
            if article.get("claim") and article.get("claim").strip():  # Only if claim exists and is not empty
                # Normalize the verdict
                normalized_verdict = normalize_verdict(article.get("verdict"), article.get("claim"))
        ...
        if len(article_dicts) != orig_article_count:
            print(f"  Deduped articles: {orig_article_count - len(article_dicts)} removed")

        orig_vec_count = len(article_vector_dicts)
        article_vector_dicts = list({v["chunk_id"]: v for v in article_vector_dicts}.values())
        if len(article_vector_dicts) != orig_vec_count:
            print(f"  Deduped vectors: {orig_vec_count - len(article_vector_dicts)} removed")
        ...
        try:
            ...
            if article_dicts:
                ...
                for i in range(0, len(article_dicts), batch_size):
                    ...
                    print(f"  ✓ Inserted/updated articles {i+1}-{min(i+batch_size, len(article_dicts))}")
            ...
            if claims_dicts:
                ...
                for i in range(0, len(claims_dicts), batch_size):
                    ...
                    session.execute(stmt)
                    print(f"  ✓ Inserted/updated claims {i+1}-{min(i+batch_size, len(claims_dicts))}")
