...
def remove_footer(text):
    """Remove common footer/link lines and 'Read the full story' sentences from content."""
    if not isinstance(text, str):
        return text

    # Remove "Read the full story..." sentences (anywhere in text, not just line starts)
    text = re.sub(
        r"\s*Read the full\s+(?:story|article|post|stories|posts|text)(?:\s+(?:on|at|here|from|via)\s+\S+)*\s*\.?\s*",
        "",
        text,
        flags=re.IGNORECASE,
    )
    ...
    cleaned_lines = [ln for ln in lines if not is_footer(ln)]
    return "\n".join(cleaned_lines).strip()


def remove_links(text):
    """Remove embedded URLs anywhere in the content."""
    if not isinstance(text, str):
        return text
...
def remove_fact_check_prefix(title):
    """
    Remove 'Fact Check: ' or 'Fact check: ' prefix from title.

    Args:
        title: The title to clean

    Returns:
        Title without the fact check prefix
    """
    if not isinstance(title, str):
        return title
    return re.sub(r"^Fact\s+[Cc]heck:\s+", "", title)
...
def clean_article(article):
    ...
    if "content" in cleaned:
        ...
        cleaned["content"] = remove_footer(cleaned["content"])
        cleaned["content"] = remove_links(cleaned["content"])

    if "title" in cleaned:
        cleaned["title"] = clean_text_content(cleaned["title"])

    if "claim" in cleaned:
        cleaned["claim"] = clean_text_content(cleaned["claim"])
...
def clean_frontpage_articles(input_path, output_path, no_verdict_output_path=None):
    ...
    for article in articles:
        ...
        if "content" in article:
            cleaned_content = remove_location_prefix(article["content"])
            cleaned_content = clean_text_content(cleaned_content)
            cleaned_content = remove_footer(cleaned_content)
            cleaned_content = remove_links(cleaned_content)
            article["content"] = clean_text_content(cleaned_content)

        if "title" in article:
            article["title"] = clean_text_content(article["title"])
            article["title"] = remove_fact_check_prefix(article["title"])
        ...
        if is_fact_check and verdict_missing and no_verdict_output_path:
            article["type"] = "FACT-CHECK-NO-VERDICT"
...
if __name__ == "__main__":
    # Define file paths
    raw_file = os.path.join(os.path.dirname(__file__), "../data/raw/articlesTsek.json")
    processed_file = os.path.join(
        os.path.dirname(__file__), "../data/processed/articlesTsek.json"
    )
    no_verdict_file = os.path.join(
        os.path.dirname(__file__), "../data/processed/articlesTsekNoVerdict.json"
    )
