...
# --- Configuration ---
MAX_PAGES = 141
LISTING_URL = "https://verafiles.org/issues-archive/2035/all/30"

# Using shared EMBEDDER from utils


def save_article(article: dict):
    return save_article_sync(article, VeraCleaner, EMBEDDER)
...
def _extract_section_by_heading(page, keywords: list) -> str | None:
    """
    Fallback helper to find content associated with a general heading/keyword
    (used if specific block extraction fails).
    """
    for kw in keywords:
        # Try matching h2/h3/h4 headings first (case-insensitive)
        xpath_h = f"xpath=//h2[contains(translate(normalize-space(.), 'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'), '{kw}')]|//h3[contains(translate(normalize-space(.), 'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'), '{kw}')]|//h4[contains(translate(normalize-space(.), 'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'), '{kw}')]"
        try:
            headings = page.locator(xpath_h)
            if headings.count() > 0:
                for i in range(headings.count()):
                    head = headings.nth(i)

                    # 1) following siblings
                    siblings = head.locator("xpath=following-sibling::*")
                    for j in range(siblings.count()):
                        node = siblings.nth(j)
                        ps = node.locator("p").all()
                        if ps:
                            content = "\n".join(
                                [
                                    p.inner_text().strip()
                    ...
                    # 2) parent container
                    parent = head.locator("xpath=parent::*")
                    if parent.count() > 0:
                        ps = parent.first.locator("p").all()
                        if ps:
                            content = "\n".join(
                                [
                                    p.inner_text().strip()
    ...
    # Broader search: any element that contains the phrase
    for kw in keywords:
        try:
            xpath_any = f"xpath=//*[contains(translate(normalize-space(.), 'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'), '{kw}')]"
            elems = page.locator(xpath_any)
            if elems.count() > 0:
                for k in range(elems.count()):
                    e = elems.nth(k)

                    # try its following siblings
                    siblings = e.locator("xpath=following-sibling::*")
                    for j in range(siblings.count()):
                        node = siblings.nth(j)
                        ps = node.locator("p").all()
                        if ps:
                    ...
                    # try descendant <p> of the element itself
                    ps = elems.first.locator("p").all()
                    if ps:
                        content = "\n".join(
                            [
                                p.inner_text().strip()
...
def _extract_fact_check_block(page, heading_text: str) -> str | None:
    """
    Specifically targets the content associated with a high-level fact-check heading
    (e.g., 'WHAT WAS CLAIMED' or 'OUR VERDICT') using the new structured container.
    This ensures the full content, including the verdict label ("False:"), is captured.
    """
    # 1. Locate the h2 element with the specific uppercase text
    xpath_h2 = f"xpath=//h2[normalize-space(.)='{heading_text}']"

    try:
        heading_locator = page.locator(xpath_h2).first
        if heading_locator.count() == 0:
            return None

        # 2. Find the immediate next sibling (the <div> that holds the text/p-tag)
        content_locator = heading_locator.locator(
            "xpath=following-sibling::div[1]"
        ).first

        if content_locator.count() > 0:
            # Use inner_text() to capture all text, including the verdict label outside the <p>
            text_content = content_locator.inner_text().strip()

            if text_content:
                # Clean up potential extra newlines or spaces
                return re.sub(r"\s{2,}", " ", text_content).strip()

    except Exception as e:
        print(f"Error in _extract_fact_check_block for {heading_text}: {e}")
        pass
...
def scrape_verafile_article(page, url: str) -> dict:
    ...
    try:
        ...
        # --- Extract Claim and Verdict (PRIORITY: New Structured Method) ---

        claim = _extract_fact_check_block(page, "WHAT WAS CLAIMED")
        verdict = _extract_fact_check_block(page, "OUR VERDICT")

        # Keywords (uppercase) for FALLBACK
        claim_keywords = ["WHAT WAS CLAIMED", "WHAT WAS SAID", "THE CLAIM", "CLAIM"]
        verdict_keywords = ["OUR VERDICT", "VERDICT", "OUR FINDING", "OUR CONCLUSION"]

        # FALLBACK LOGIC
        if not claim:
            claim = _extract_section_by_heading(page, claim_keywords)
        if not verdict:
            verdict = _extract_section_by_heading(page, verdict_keywords)
        ...
        source = domain.split(".")[0] if domain else "verafiles"
...
def run_scraper(listing_url: str):
    ...
    print(f"Starting Vera Files FC scraper with real-time DB integration...")
...
async def main():
    await asyncio.to_thread(run_scraper, LISTING_URL)
